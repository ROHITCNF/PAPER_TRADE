// Detects thin order book + wide spread — signals potential rapid/erratic price move.
// detect() returns true when:
//   - current total depth < 60% of rolling average  (book dried up)
//   - current spread > 120% of rolling average       (spread widened)
function createLiquidityVacuum(window = 10) {
    ...
    function detect() {
        ...
        const avgDepth  = depthHistory.reduce((a, b) => a + b, 0) / window;
        const avgSpread = spreadHistory.reduce((a, b) => a + b, 0) / window;
